...
const t = (data?.users ?? []).filter((u) => u.email?.startsWith("t6b_"));
...
console.log("deleted", t.length, "| remaining t6b_ users:", (after?.users ?? []).filter((u) => u.email?.startsWith("t6b_")).length);
